...
module.exports.connect = function googleStackdriverConnect( emitter, userConfig ) {
  ...
  var keyFile = process.env['GOOGLE_APPLICATION_CREDENTIALS'] || config.keyFile || '/etc/google/auth/application_default_credentials.json';
  try {
  ...
  backend.sendPayload = function sendPayload( payload, context ) {
    ...
    googleMetrics.uploadCustomMetricsToGoogleStackdriver(creds, metricsBatches, function(err, replies) {
      if (err) return self.emitter.emit('error', 'Google Stackdriver upload error: ' + err.message);
      for (var i=0; i<replies.length; i++) {
        if (replies[i].statusCode >= 400) {
          ...
          self.emitter.emit('error', replies[i].statusCode + ' error sending to Google Stackdriver: ' + message);
        }
      }
    });
  }
