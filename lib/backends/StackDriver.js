...
exports.connect = function(emitter, config) {
  ...
  StackDriverBackend.prototype.processStats = function(stats) {
    ...
    var payload = {
      timestamp : stats.lastFlushTimeUnix,
      proto_version : 1,
      data : []
    };
  ...
  StackDriverBackend.prototype.sendPayload = function(payload, context) {
    ...
    var options = {
      host : context.stackDriverHost,
      path : context.stackDriverPath,
      method : 'POST',
      headers : {
        "Content-Length" : message.length,
        "Content-Type" : "application/json; charset=utf-8",
        "x-stackdriver-apikey" : context.apiKey,
    ...
    var req = https.request(options, function(res) {
      ...
      if (Math.floor(res.statusCode / 100) === 4 || Math.floor(res.statusCode / 100) === 5){
        context.emitter.emit('error', res.statusCode + " error sending to Stackdriver");
      }
  ...
  StackDriverBackend.prototype.addStat = function (name, stats, payload) {
    ...
    var stackDriverStat = {
      ...
      value: stats[name],
      collected_at: stats.lastFlushTimeUnix
    };
